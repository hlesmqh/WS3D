...
class Calibration(object):
    def __init__(self, calib_file):
        ...
        self.rect_4x4 = np.zeros([4, 4], dtype = self.R0.dtype)
        self.rect_4x4[3, 3] = 1.
        self.rect_4x4[:3, :3] = self.R0     # 4 x 4

        self.V2C_4x4 = np.concatenate([self.V2C, np.array([[0., 0., 0., 1.]])], axis=0)
    ...
    def ry_2alpha(self, kx, kz, ry):
        # change kitti roty to observation angle alpha
        old_x, old_z, ry = kx, kz, ry
        old_beta = np.arctan2(old_z, old_x)
        alpha = -np.sign(old_beta) * np.pi / 2 + old_beta + ry
        return alpha


    def box_center_camera_to_lidar(self, center):

        center.reshape((1, 3))
        points_shape = list(center.shape[:-1])
        center = np.concatenate([center, np.ones(points_shape + [1])], axis=-1)

        lidar_box_center = center @ np.linalg.inv((self.rect_4x4 @ self.V2C_4x4).T)
        return lidar_box_center[:3]

    def box_center_lidar_to_camera(self, center):


        center = center.reshape((1, 3))
        points_shape = list(center.shape[:-1])
        center = np.concatenate([center, np.ones(points_shape + [1])], axis=-1)
        lidar_box_center = center @ (self.rect_4x4 @ self.V2C_4x4).T
        return lidar_box_center[:3]
    ...
    def camera_to_lidar(self, points):
        r_rect = self.rect_4x4
        velo2cam = self.V2C_4x4
    ...
    def lidar_to_image(self, points):
        point_rect = self.lidar_to_camera(points)
        return self.project_to_image(point_rect)
