...
def decode_center_target(roi_center, pred_reg, loc_scope, loc_bin_size):
    """
    :param roi_box3d: (N, 3)
    :param pred_reg: (N, C)
    :param loc_scope:
    ...
    x_res = x_res_norm * (loc_bin_size / 2)
    z_res = z_res_norm * (loc_bin_size / 2)
    pos_x += x_res
    pos_z += z_res
    ...
    shift_ret_box3d = torch.cat((pos_x.view(-1, 1), pos_y.view(-1, 1), pos_z.view(-1, 1)), dim=1)
    ret_box3d = shift_ret_box3d
    ret_box3d[:, [0, 2]] += roi_center[:, [0, 2]]
    return ret_box3d


def decode_bbox_target_stage_2(roi_box3d, pred_reg, loc_scope, loc_bin_size, num_head_bin, anchor_size,
...
def decode_bbox_target(roi_box3d, pred_reg, loc_scope, loc_bin_size, num_head_bin, anchor_size,
                       get_xz_fine=False, get_y_by_bin=False, loc_y_scope=0.5, loc_y_bin_size=0.25, get_ry_fine=False):
    """
    :param roi_box3d: (N, 7)
    :param pred_reg: (N, C)
    :param loc_scope:
    ...
    #Log norm size
    # size_res_norm = pred_reg[:, size_res_l: size_res_r]
    # hwl = torch.exp(size_res_norm) * anchor_size
